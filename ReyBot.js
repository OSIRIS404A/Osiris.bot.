// © Copyright REYOSIRISOFC
// Bot By OSIRIS
// FAVOR DE NO MODIFICAR CODIGOS DE SER ASI ME DARE CUENTA
...
async function starts() {
	const ReyBot = new WAConnection()
	ReyBot.version = [2, 2119, 6]
	ReyBot.logger.level = 'warn'
	console.log(banner.string)
	console.log(color('[ BOT]','aqua'), color("BOT BY REY OSIRIS", "yellow"))
	console.log('>', '[',color('INFO','blue'),']','ESTE ES EL BOT OFICIAL DE DEATH HOUSE')
	ReyBot.on('qr', () => {
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' Escanea El Código QR De Arriba'))
	})

	fs.existsSync('./session.json') && ReyBot.loadAuthInfo('./session.json')
	ReyBot.on('connecting', () => {
	console.log(color('> [ INFO ]', 'white'), color('Conectando...'))
	})
	ReyBot.on('open', () => {
	console.log(color('> [ INFO ]', 'white'), color('Ya estas conectado'))
	})
		await ReyBot.connect({timeoutMs: 30*1000})
  fs.writeFileSync('./session.json', JSON.stringify(ReyBot.base64EncodedAuthInfo(), null, '\t'))
//Banned Call
ReyBot.on('CB:action,,call', async json => {
    const callerId = json[2][0][1].from;
    console.log("call dari "+ callerId)
        ReyBot.sendMessage(callerId, "Sistema De Bloqueo Automático, NO LLAMES POR FAVOR", MessageType.text)
        await sleep(4000)
        await ReyBot.blockUser(callerId, "add")
})
//Welkom
ReyBot.on('group-participants-update', async (anu) => {
		if (!welkom.includes(anu.jid)) return
		try {
			const mdata = await ReyBot.groupMetadata(anu.jid)
			console.log(anu)
			if (anu.action == 'add') {
				num = anu.participants[0]
				try {
					ppimg = await ReyBot.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
				} catch {
...
┗━━━━━━━━━━━━━━━━━
┏━━━━━━━━━━━━━━━━━
┠⊷️ *UNETE A NUESTRO GRUPO DE FACEBOOK*:
┠⊷️ *https://www.facebook.com/groups/501732644413094/?ref=share*
┠⊷️ *Grupo RCT*:
┠⊷️ *https://chat.whatsapp.com/G1Dl7apujok4lcmuqjrbQC
┗━━━━━━━━━━━━━━━━━
┏━━━━━━━━━━━━━━━━━
┠⊷️「 *DESCRIPCIÓN* 」
┗━━━━━━━━━━━━━━━━━
...
┗━━━━━━━━━━━━━━━━━`
				...
				ReyBot.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				try {
					ppimg = await ReyBot.getProfilePicture(`${num.split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				teks = `*@${num.split('@')[0]}*
...
${promote}`
			ReyBot.sendMessage(mdata.id, buff, MessageType.image, {caption : teks, contextInfo: {mentionedJid: [num]}, quoted: { "key": { "participant": `${numbernye}`, "remoteJid": `Kntl`, "fromMe": false, "id": "B391837A58338BA8186C47E51FFDFD4A" }, "message": { "documentMessage": { "jpegThumbnail": buff, "mimetype": "application/octet-stream", "title": `PROMOTE`, "fileLength": "36", "pageCount": 0, "fileName": `_Welcome_` }}, "messageTimestamp": "1614069378", "status": "PENDING"}})
...
${demote}`
			ReyBot.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {mentionedJid: [num]}, quoted: { "key": { "participant": `${numbernye}`, "remoteJid": `Ktl`, "fromMe": false, "id": "B391837A58338BA8186C47E51FFDFD4A" }, "message": { "documentMessage": { "jpegThumbnail": buff, "mimetype": "application/octet-stream", "title": `DEMOTE`, "fileLength": "36", "pageCount": 0, "fileName": `_Welcome_` }}, "messageTimestamp": "1614069378", "status": "PENDING"}})
		}
		} catch (e) {
...
ReyBot.on('chat-update', async (mek) => {
	try {
		...
		const q = args.join(' ')
		const meNumber = ReyBot.user.jid.split("@")[0]
		const botNumber = ReyBot.user.jid
//SETTING
		...
		const totalchat = await ReyBot.chats.all()
		const groupMetadata = isGroup ? await ReyBot.groupMetadata(from) : ''
        ...
        const conts = mek.key.fromMe ? ReyBot.user.jid : ReyBot.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = mek.key.fromMe ? ReyBot.user.name : conts.notify || conts.vname || conts.name || '-'
       //SETTINGS FUNCIONES
       const apakah = ['Si','No']
       ...
       const kapankah = ['Otro día','Otra semana','Otro mes','Otro año']
        ...
        const reply = (teks) => {
            ReyBot.sendMessage(from, teks, text, {quoted:mek})
        }

        const sendMess = (hehe, teks) => {
            ReyBot.sendMessage(hehe, teks, text, {sendEphemeral: true})
        }

        const mentions = (teks, memberr, id) => {
            (id == null || id == undefined || id == false) ? ReyBot.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : ReyBot.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": memberr } })
        }
        const costum = (pesan, tipe, target, target2) => {
                 ReyBot.sendMessage(from, pesan, tipe, {quoted: {key: {fromMe: false, participant: `${target}`, ...(from ? {remoteJid: from}: {})}, message: {conversation: `${target2}` }}})
        }
        //FAKEH
const fakekontak = (teks) => {
ReyBot.sendMessage(from, teks, MessageType.text, {
quoted: {
key: {
fromMe: false,
...
'vcard': `BEGIN:VCARD\nVERSION:3.0\nN:XL;${ucapanWaktu},;;;\nFN:${ucapanWaktu},\nitem1.TEL;waid=${sender.split('@')[0]}:${sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`,
 'jpegThumbnail': fs.readFileSync('./stik/thumb.jpeg')
        ...
        const fakethumb = (teks, yes) => {
            ReyBot.sendMessage(from, teks, image, {thumbnail:fs.readFileSync('./stik/fake.jpeg'),quoted:mek,caption:yes})
        }
        const fakestatus = (teks) => {
            ReyBot.sendMessage(from, teks, text, {
                quoted: {
                    key: {
                        fromMe: false,
	  ...
	  contextInfo:{
        ...
        const sendStickerFromUrl = async(to, url) => {
                ...
                download(url, './stik' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${filess} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${asw}`, (err) => {
                        ...
                        ReyBot.sendMessage(to, media, MessageType.sticker,{quoted:mek})
        ...
        const sendMediaURL = async(to, url, text="", mids=[]) =>{
                ...
                download(url, filename, async function () {
                    ...
                    ReyBot.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
...
//========================================================================================================================//
  ...
  wew = fs.readFileSync('./src/foto.png')
					ReyBot.sendMessage(from, wew, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "Felixcrack BOT Ya esta verificado.", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('./src/botlogo.webp')} } }, caption: help(prefix) })
				  break
case prefix+ 'idioma':
ReyBot.sendMessage(from, idioma(prefix, sender), text, {quoted: mek})
break
case prefix+ 'yomenu':
ReyBot.sendMessage(from, toinmenu(prefix, sender), text, {quoted: mek})
break
case prefix+ 'menuadmin':
ReyBot.sendMessage(from, menuadmin(prefix, sender), text, {quoted: mek})
break
case prefix+ 'nsfwmenu':
if (!isNsfw) return reply(mess.nsfwoff)
ReyBot.sendMessage(from, nsfwmenu(prefix, sender), text, {quoted: mek})
break
case prefix+ 'desmenu':
ReyBot.sendMessage(from, desmenu(prefix, sender), text, {quoted: mek})
break
case prefix+ 'versión':
case prefix+ 'version':
ReyBot.sendMessage(from, version(prefix, sender), text, {quoted: mek})
break
case prefix+ 'welmenu':
ReyBot.sendMessage(from, welmenu(prefix, sender), text, {quoted: mek})
break
...
case prefix+ 'gtts':
case 'tts':
if (args.length < 1) return ReyBot.sendMessage(from, `Debes usar el comando correctamente:\n${prefix}tts (lengua) (texto)\nEjemplo: ${prefix}tts Buen dia abortos\n\nUse: ${prefix}ts para enumerar todos los idiomas`, text, {quoted: mek})
const gtts = require('./lib/gtts')(args[0])
if (args.length < 2) return ReyBot.sendMessage(from, 'Y El Texto?', text, {quoted: mek})
dtt = body.slice(8)
ranm = getRandom('.mp3')
dtt.length > 800
? reply('¿Quieres Escribir La Biblia?')
: gtts.save(ranm, dtt, function() {
ReyBot.sendMessage(from, fs.readFileSync(ranm), audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
fs.unlinkSync(ranm)
})
break
...
case prefix+ 'hidetag':
			...
			var value = args.join(' ')
			var group = await ReyBot.groupMetadata(from)
			...
			ReyBot.sendMessage(from, optionshidetag, text)
			break
    case prefix+ 'sticktag':
            if ((isMedia && !mek.message.videoMessage || isQuotedSticker) && args.length == 0) {
            encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
            file = await ReyBot.downloadAndSaveMediaMessage(encmedia, filename = getRandom())
            value = args.join(" ")
            var group = await ReyBot.groupMetadata(from)
            var member = group['participants']
            var mem = []
            ...
            ReyBot.sendMessage(from, ini_buffer, MessageType.sticker, options)
            fs.unlinkSync(file)
            } else {
		    ...
		    case prefix+ 'giftag':
                ...
                download = await ReyBot.downloadMediaMessage(quoted)
                ...
                ReyBot.sendMessage(from, thumb, video, { contextInfo: {mentionedJid: mem }, quoted: mek, mimetype: 'video/gif', thumbnail: thumb })
			...
			case prefix+ 'doctag':
			  //by Dehanjing
                ...
                ReyBot.sendMessage(from, fs.readFileSync(`doc.txt`), document, { contextInfo: {mentionedJid: mem }, quoted: mek, mimetype: 'text/plain' })
    ...
    case prefix+ 'contag':
            ...
            + 'END:VCARD'.trim()
            ReyBot.sendMessage(from, {displayName: `${nah}`, vcard: vcard}, contact, {contextInfo: {"mentionedJid": members_ids}})
            break
    case prefix+ 'totag':
            if ((isMedia && !mek.message.videoMessage || isQuotedSticker) && args.length == 0) {
            encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
            ...
            ini_buffer = fs.readFileSync(file)
            ReyBot.sendMessage(from, ini_buffer, image, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            ReyBot.sendMessage(from, ini_buffer, audio, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            ReyBot.sendMessage(from, ini_buffer, video, options)
            fs.unlinkSync(file)
        } else{
...
case prefix+ 'closegc':
case 'closegp':
ReyBot.updatePresence(from, Presence.composing)
if (!isGroup) return reply(mess.only.group)
...
ReyBot.groupSettingChange (from, GroupSettingChange.messageSend, true);
reply(close)
break
case prefix+ 'opengc':
case 'opengp':
...
ReyBot.groupSettingChange (from, GroupSettingChange.messageSend, false);
reply(open)
break
case prefix+ 'kick':
case prefix+ 'pafuera':
if (!isGroup) return reply(mess.only.group)
					...
					ReyBot.groupRemove(from, mentioned)
					} else {
                 ...
                 case prefix+ 'pm':
					...
					if (mentioned.length > 1) {
						...
						mentions(teks, mentioned, true)
						ReyBot.groupMakeAdmin(from, mentioned)
					} else {
					...
					if (!isGroup) return reply(mess.only.group)
				    if (!isBotGroupAdmins) return reply(`「❗」ESTE COMANDO SOLO SE PUEDE USAR CUANDO EL BOT ES ADMIN`)
				    linkgc = await ReyBot.groupInviteCode (from)
				    yeh = `https://chat.whatsapp.com/${linkgc}\n\nLink Del Grupo *${groupName}*`
				    ReyBot.sendMessage(from, yeh, text, {quoted: mek1})			       
					break
case prefix+ 'leave':
                    if (!isGroup) return reply(mess.only.group)
                    if (!isGroupAdmins) return reply(mess.only.admin)
                     setTimeout( () => {
					ReyBot.groupLeave (from) 
					}, 2000)
                     setTimeout( () => {
					ReyBot.updatePresence(from, Presence.composing) 
					ReyBot.sendMessage(from, 'Adios jotos👋', text) // ur cods
					}, 0)
                     break
       case prefix+ 'ownergrup':
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           ReyBot.sendMessage(from, options, text, { quoted: mek } )
				break
...
case prefix+ 'tagall2':
				...
				ReyBot.updatePresence(from, Presence.composing) 
					...
					ReyBot.sendMessage(from, '𝙈𝙚𝙣𝙘𝙞𝙤𝙣 𝘿𝙚 𝙏𝙤𝙙𝙤𝙨'+teks+'✪ FX ┃ ᴮᴼᵀ ✪', text, {quoted: mek})
					break
                case prefix+ 'tagall3':
                case prefix+ 'marcar3':
				ReyBot.updatePresence(from, Presence.composing) 
					...
					ReyBot.sendMessage(from, '𝙼𝙴𝙽𝙲𝙸𝙾𝙽 𝙳𝙴 𝚃𝙾𝙳𝙾𝚂'+teks+'', text, {detectLinks: false, quoted: mek})
					break
                        case prefix+ 'tagall4':
                        case prefix+ 'marcar4':
				ReyBot.updatePresence(from, Presence.composing) 
					...
					ReyBot.sendMessage(from, '𝙈𝙚𝙣𝙘𝙞𝙤𝙣 𝘿𝙚 𝙏𝙤𝙙𝙤𝙨'+teks+'✔︎', text, {quoted: mek})
					break
                case prefix+ 'tagall5':
                case prefix+ 'marcar5':
				ReyBot.updatePresence(from, Presence.composing) 
...
case prefix+ 'tovid':
    case prefix+ 'tovideo':
            ...
            owgi = await ReyBot.downloadAndSaveMediaMessage(ger)
    ...
    case prefix+ 'tomp3':
            ...
            media = await ReyBot.downloadAndSaveMediaMessage(encmedia)
            ran = getRandom('.mp4')
            exec(`ffmpeg -i ${media} ${ran}`, (err) => {
            fs.unlinkSync(media)
            if (err) return fakegroup(`Err: ${err}`)
            buffer453 = fs.readFileSync(ran)
            ReyBot.sendMessage(from, buffer453, audio, { mimetype: 'audio/mp4', quoted: mek })
            fs.unlinkSync(ran)
            })
            break
    case prefix+ 'toimg':
      ...
      rano = getRandom('.webp')
			...
			exec(`wget ${anu.data.url} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
			  fs.unlinkSync(ranp)
				...
				ReyBot.sendMessage(from, buffer, MessageType.sticker, {quoted: mek})
				fs.unlinkSync(rano)
			})
			break
...
if (!isNsfw) return reply(mess.nsfwoff)
  pusiimg = await axios.get('https://nekos.life/api/v2/img/pussy_jpg')
			bufpusy = await getBuffer(pusiimg.data.url)
				ReyBot.sendMessage(from, bufpusy, MessageType.image, {quoted: mek})
			.catch(err => {
			return('Error 😔..')
			})
			break
...
if (!isNsfw) return reply(mess.nsfwoff)
			...
			ReyBot.sendMessage(from, bupyuri, image, { quoted: mek })
			.catch(err => {
			return('Error 😔..')
			})
			break
...
if (!isNsfw) return reply(mess.nsfwoff)
			...
			ReyBot.sendMessage(from, buferon, image, { quoted: mek })
			.catch(err => {
			return('Error 😔..')
			})
			break
...
if (!isNsfw) return reply(mess.nsfwoff)
			...
			ReyBot.sendMessage(from, bufero, image, { quoted: mek })
			.catch(err => {
...
if (!isNsfw) return reply(mess.nsfwoff)
	...
	if (!isNsfw) return reply(mess.nsfwoff)
			...
			ReyBot.sendMessage(from, bufhtz, image, {quoted: mek})
			.catch(err => {
			return('Error 😔..')
			})
			break
...
case prefix+ 'stickergif': 
    ...
    case prefix+ 's':
            ...
            const media = await ReyBot.downloadAndSaveMediaMessage(encmedia)
                ran = '666.webp'
                await ffmpeg(`./${media}`)
                .input(media)
                ...
                .on('end', function () {
                console.log('Finish')
                ReyBot.sendMessage(from, fs.readFileSync(ran), MessageType.sticker, {quoted: mek})
                 fs.unlinkSync(media)
                fs.unlinkSync(ran)
                })
...
ReyBot.sendMessage(from, attp2, MessageType.sticker, selepbot, {quoted: mek })
						break
...
break
       ...
       //OWNER BOT
            ...
            if (!mek.key.fromMe) return fakestatus('「 ❗ 」ESTE COMANDO SOLO PUEDE SER USADO POR MI')
            var chats = await ReyBot.chats.all()
            chats.map( async ({ jid }) => {
            await ReyBot.chatRead(jid)
            })
		    var teks = `\`\`\`Fueron Leídos ${chats.length} Chats !\`\`\``
	        await ReyBot.sendMessage(from, teks, text, {quoted: mek})
...
reply(mess.mpv)
	...
	case prefix+ 'ytsearch':
        	...
        	return await ReyBot.sendMessage(from, 'Error!', MessageType.text, dload)
    		}
    		aramat = aramas.all 
...
case prefix+ 'play':
    ...
    ReyBot.sendMessage(from, options, text, { quoted: mek } )
				break
case prefix+ 'creador':
await ReyBot.sendMessage(from, {displayname: "Félix🧙‍♂️", vcard: vcard}, MessageType.contact)
break
